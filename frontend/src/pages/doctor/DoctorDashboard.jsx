    ...
    <div className="page-wrapper">
      <div className="container">
        <div className="page-header">
          <h1>Hello, Dr. {user?.name?.split(' ').pop()} 👨‍⚕️</h1>
          {profile && !profile.isVerified && (
            <div className="error-msg" style={{ marginTop: 10, display: 'inline-block' }}>
              ...
              {today.map(a => (
                <div key={a._id} className="appt-row">
                  <div className="appt-row-left">
                    <div className="appt-doctor-initial">{a.patientName?.charAt(0)}</div>
                    <div>
                      <div className="appt-doctor-name">{a.patientName}</div>
                      <div className="appt-meta">
                        {a.timeSlot?.startTime} · {a.type === 'telemedicine' ? '📹 Video' : '🏥 In-person'}
                      </div>
                    </div>
                  </div>
                  <div className="appt-row-right">
                    ...
                    {a.status === 'confirmed' && a.videoRoomId && (
                      <a href={`https://meet.jit.si/${a.videoRoomId}`} target="_blank" rel="noreferrer"
                        className="btn btn-accent btn-sm" style={{ marginTop: 4 }}>
                        Join
                      </a>
                    )}
                  </div>
                ...
                <div key={a._id} className="appt-row">
                  <div className="appt-row-left">
                    ...
                    <div>
                      <div className="appt-doctor-name">{a.patientName}</div>
                      <div className="appt-meta">
                        {new Date(a.appointmentDate).toLocaleDateString()} · {a.timeSlot?.startTime}
                      </div>
                    </div>
                  </div>
